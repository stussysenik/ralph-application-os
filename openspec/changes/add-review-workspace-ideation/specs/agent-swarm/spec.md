...
#### Scenario: Ideate a screenshot review prompt
- **WHEN** the operator runs the ideation command or interview command with a screenshot, annotation, collection, and review-link oriented prompt
- **THEN** Ralph SHALL classify the prompt as `review-workspace`
- **AND** Ralph SHALL keep the prompt on the interactive-runtime path
- **AND** the generated interview questions SHALL include asset lifecycle and annotation or sharing pressure
